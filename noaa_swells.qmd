...
winter_swells <- swell_data %>% 
  ...
  mutate(year = year(time), # make a year column
         month = month(time), # make a  month column
         day = day(time), # make a day column
         wave_height = round(wave_height*feet_per_meter, 2)) %>% # convert wave_height from meters to feet so it is easier to interpret
  group_by(date) %>% 
...
```{r}
ggplot(data = winter_swells, aes(x = time, y = wave_height, group = year)) +
  geom_line()
```
